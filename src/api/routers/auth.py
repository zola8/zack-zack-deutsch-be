...
def frontend_redirect(path: str, params: dict | None = None) -> RedirectResponse:
    return RedirectResponse(url=build_frontend_url(path, params))
...
async def google_login(request: Request):
    ...
    logger.info("Initiating Google OAuth login. Redirect URI: %s", redirect_uri)
...
async def google_callback(request: Request, db: Session = Depends(get_db)):
    ...
    if google_error:
        ...
        return frontend_redirect(settings.FRONTEND_CALLBACK_PATH, {"error": google_error})

    # 2. Exchange code for Google access token
    try:
        token = await oauth.google.authorize_access_token(request)
    except Exception as e:
        logger.error("Failed to authorize access token: %s", e, exc_info=True)
        return frontend_redirect(settings.FRONTEND_CALLBACK_PATH, {"error": "auth_failed"})

    # 3. Parse Google ID token / userinfo
    try:
        userinfo = await oauth.google.parse_id_token(token, nonce=None)
    except Exception as e:
        logger.error("Failed to parse ID token: %s", e, exc_info=True)
        return frontend_redirect(settings.FRONTEND_CALLBACK_PATH, {"error": "no_userinfo"})
    ...
    try:
        ...
        return frontend_redirect(settings.FRONTEND_CALLBACK_PATH, {"error": "user_failed"})

    # 5. Redirect to frontend with HttpOnly cookie
    response = frontend_redirect(settings.FRONTEND_CALLBACK_PATH, {"status": "success"})
